...
Web.ImageViewer.prototype = {
	...
	resize: function() {
		$('unc_popup_img').src = $('unc_popup_hidn').src;
		$('unc_image_viewer').style.width = $('unc_popup_img').width+6;
		$('unc_image_viewer').style.height = $('unc_popup_img').height+35;

		this.el.style.left = ((Web.util.Position.clientWidth()/2) - (($('unc_popup_img').width+6)/2)) + 'px';
		this.el.style.top = ($('unc_popup_img').height+35) > Web.util.Position.clientHeight() ? Web.util.Position.deltaY() : (Web.util.Position.deltaY()+((Web.util.Position.clientHeight()-($('unc_popup_img').height+35))/2))+'px';
	},
	generate: function(args) {		
		...
		'<tr bgcolor=#ffffff>'+
			...
			'<td class=w-viewer-mm>'+
				'<table border=0 cellspacing=0 cellpadding=0 width=100%>'+
					'<tr>'+
						'<td class=w-viewer-mm-txt align=center><img title="클릭하면 이미지가 닫힙니다" style=cursor:pointer src='+Web.config.imageroot+'oz/window/i_large_loading.gif name=unc_popup_img id=unc_popup_img onclick=\'Web.CanvasManager.get("'+this.el.id+'").hide();\'>'+
		...
		this.update(div);
		$('unc_image_viewer').style.width = '300px';
		$('unc_image_viewer').style.height = '400px';
	}
};
...
Web.Slider = {
	...
	load: function(arr) {
		...
		$('unc-slider').update(txt);
		
		this.slider(103, 1);
	},
	slider: function(w, cnt) {
		this.iwidth = w * cnt;
		var groups = $('imageSlider').getElementsByTagName('div');
		...
		var btns = $('imageSlider').getElementsByTagName('img');
	...
	next: function(cnt) {
		if (this.index < this.length-1) {
			...
			Web.message({title:'알림',msg:'마지막 이미지입니다',height:80});
		}
	},
	prev: function(cnt) {
		if (this.index > 0) {
			...
			Web.message({title:'알림',msg:'처음 이미지입니다',height:80});
		}
	},
	toggle: function() {
		if (this.visible) {
	...
	show: function() {
		$('unc-slider').style.display = 'inline';
	},
	hide: function() {
		$('unc-slider').style.display = 'none';
	}
};
