...
<!-- textarea name하고 "test"부분하고 똑같아야 함  --> 
function writeArticle(){
	if(document.writeForm.subject.value == ""){
		...
		document.writeForm.action = "<%=root%>/reboard"; <!-- form이니까 hidden 부분에서 값 보내기 -->
...
<!-- 모든 게시판 controller에는 act(기능), bcode(게시판번호), pg(페이지번호), key(검색 조건), word(검색어) -->
<input type="hidden" name="act" value="write">
<input type="hidden" name="bcode" value="<%=bcode%>">
<input type="hidden" name="pg" value="1"> <!-- 새글 작성이니까 페이지는 1페이지로 가면 됨 -->>
<input type="hidden" name="key" value=""> <!-- 새글 작성이므로 검색조건 검색어 필요없음 -->
